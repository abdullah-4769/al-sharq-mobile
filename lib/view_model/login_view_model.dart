...
//       await SharedPrefsHelper.saveUserOrganization(result.user.organization);
//
//       // Save photo - try multiple fields
//       final photo = result.user.photo ?? result.user.picUrl;
//       await SharedPrefsHelper.saveUserPhoto(photo);
//
//       // Save latest event ID if available
...
//       await SharedPrefsHelper.saveRememberMe(rememberMe.value);
//
//       // Save speaker ID if available
//       if (result.user.speakerId != null) {
//         await SharedPrefsHelper.saveSpeakerId(result.user.speakerId!);
//       }
//
//       // ✅ FIXED: Save sponsor ID if available - This is the correct place
//       if (result.user.sponsorId != null) {
//         await SharedPrefsHelper.saveSponsorId(result.user.sponsorId!);
//         print('✅ Sponsor ID saved: ${result.user.sponsorId}');
//       } else {
//         print('⚠️ No sponsor ID found in response');
//         // Check if we can get sponsor ID from other fields
//         if (result.user.id != null && (result.user.role == 'sponsor' || result.user.role == 'exhibitor')) {
//           // For sponsor/exhibitor roles, sometimes the user ID is the sponsor ID
//           await SharedPrefsHelper.saveSponsorId(result.user.id!);
//           print('✅ Using user ID as sponsor ID: ${result.user.id}');
//         }
//       }
//
//       // Save user image if available - try multiple fields
//       final userImage = result.user.file ?? result.user.picUrl;
//       if (userImage != null && userImage.isNotEmpty) {
//         await SharedPrefsHelper.setUserImage(userImage);
//       }
//
...
//       print('   - Latest Event ID: ${result.latestEventId}');
//       print('   - Speaker ID: ${result.user.speakerId}');
//       print('   - Sponsor ID: ${result.user.sponsorId}');
//
//       // ✅ Debug: Verify sponsor ID was saved
//       final savedSponsorId = await SharedPrefsHelper.getSponsorId();
//       print('🔍 Verified saved sponsor ID: $savedSponsorId');
//
//     } catch (e) {
//       print('❌ Error saving user data: $e');
//       rethrow; // Re-throw to handle in the login method
//     }
//   }
//
//   void _navigateBasedOnRole(String role) {
...
//
//     if (error.contains('Token is missing') ||
//         error.contains('User ID is missing') ||
//         error.contains('User role is missing')) {
//       return 'Invalid server response. Please try again.';
//     } else if (error.contains('HTTP 401') || error.contains('Invalid credentials')) {
//       return 'Email or password is incorrect. Please try again.';
//     } else if (error.contains('HTTP 404')) {
//       return 'Service not available. Please try again later.';
//     } else if (error.contains('HTTP 500')) {
...
//       'userId': await SharedPrefsHelper.getUserId() ?? 0,
//       'speakerId': await SharedPrefsHelper.getSpeakerId(),
//       'sponsorId': await SharedPrefsHelper.getSponsorId(), // Added sponsorId
//     };
//   }
//
//   // Initialize current role from shared preferences
...
//   }
//
//   // ✅ NEW: Method to get sponsor ID specifically
//   Future<int?> getSponsorId() async {
//     return await SharedPrefsHelper.getSponsorId();
//   }
//
//   // ✅ NEW: Method to check if user is sponsor/exhibitor
//   Future<bool> isSponsorOrExhibitor() async {
//     final role = await SharedPrefsHelper.getUserRole();
//     final sponsorId = await SharedPrefsHelper.getSponsorId();
//     return (role == 'sponsor' || role == 'exhibitor') && sponsorId != null;
//   }
//
//   // ✅ NEW: Method to verify sponsor data is available
//   Future<void> verifySponsorData() async {
//     final sponsorId = await SharedPrefsHelper.getSponsorId();
//     final role = await SharedPrefsHelper.getUserRole();
//
//     print('🔍 Sponsor Data Verification:');
//     print('   - Role: $role');
//     print('   - Sponsor ID: $sponsorId');
//
//     if ((role == 'sponsor' || role == 'exhibitor') && sponsorId == null) {
//       print('❌ WARNING: Sponsor/Exhibitor role but no sponsor ID found!');
//     }
//   }
// }
//
