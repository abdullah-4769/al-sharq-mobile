...
class _OrganizerDashboardScreenState extends State<OrganizerDashboardScreen> with TickerProviderStateMixin {
  ...
  void initState() {
    ...
    dashboardVM = Get.put(DashboardViewModel());

    // Fetch data after frame is built
    WidgetsBinding.instance.addPostFrameCallback((_) {
      if (dashboardVM.dashboardData.value == null) {
  ...
  String _generateCSV() {
    ...
    for (int i = 0; i < data.topSessions.length; i++) {
      ...
      final speakers = session.speakers.isEmpty ? 'No speakers' : session.speakers.join(', ');
      csv.writeln('${i + 1},"${session.title}","$speakers",${session.totalRegistrations}');
    }
    csv.writeln('');
          ...
          Row(
            children: [
              Container(
                ...
                child: const Icon(Icons.bar_chart, color: Colors.redAccent, size: 20),
              ),
            ...
            child: SfCircularChart(
              ...
              series: <PieSeries<Map<String, dynamic>, String>>[
                PieSeries<Map<String, dynamic>, String>(
                  dataSource: const [
                    {'category': 'Participants', 'value': 78, 'color': Colors.redAccent},
                    {'category': 'Speakers', 'value': 15, 'color': Colors.green},
                    {'category': 'Sponsors', 'value': 7, 'color': Colors.orange},
                  ],
      ...
      child: Column(
        ...
        children: [
          ...
          ...sessions.asMap().entries.map((entry) {
            ...
            // Handle empty speakers list properly
            final speakerText = session.speakers.isEmpty
