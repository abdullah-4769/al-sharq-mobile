...
class _AddNewVenueScreenState extends State<AddNewVenueScreen> {
  ...
  void initState() {
    super.initState();
    print('📍 AddNewVenueScreen initialized - EventId: ${widget.eventId}');
    _loadInitialData();
  }
  ...
  void _loadInitialData() async {
    ...
    try {
      ...
      if (widget.eventId != null) {
        print('✏️ Edit mode - Loading event data for ID: ${widget.eventId}');
        await _loadEventData();
      } else {
        print('➕ Add mode - Starting fresh');
      }
    } finally {
      setState(() {
  ...
  Future<void> _selectEndTime() async {
    ...
    if (picked != null) {
      ...
      if (time != null) {
        ...
        print('📅 End time selected: ${endTimeController.text}');
      }
    }
  }

  Future<void> _loadSponsorsAndExhibitors() async {
    print('📦 Loading sponsors and exhibitors...');
    await Future.wait([
      _sponsorsViewModel.getSponsorsList(),
      _exhibitorsViewModel.getExhibitorsList(),
    ]);
    print('✅ Sponsors: ${_sponsorsViewModel.sponsorsList.length}, Exhibitors: ${_exhibitorsViewModel.exhibitorsList.length}');
  }

  Future<void> _loadEventData() async {
    try {
      print('🔍 Fetching event data for ID: ${widget.eventId}');
      ...
      print('✅ Event data loaded: ${event.title}');
      print('📊 Sponsors: ${selectedSponsors.length}, Exhibitors: ${selectedExhibitors.length}');
    } catch (e) {
      ...
      child: DropdownButtonFormField<OrganizerShowListSponsorsModel>(
        ...
        onChanged: (OrganizerShowListSponsorsModel? selectedSponsor) {
          if (selectedSponsor != null && !selectedSponsors.contains(selectedSponsor.id)) {
            ...
            print('✅ Sponsor added: ${selectedSponsor.name}');
          }
        },
      ),
    );
                ...
                children: [
                  ...
                  GestureDetector(
                    onTap: () {
                      ...
                      print('❌ Sponsor removed: ${sponsor.name}');
                    },
                    child: Icon(
                      Icons.close,
                      size: 14,
      ...
      child: DropdownButtonFormField<OrganizerShowListExhibitorsModel>(
        ...
        onChanged: (OrganizerShowListExhibitorsModel? selectedExhibitor) {
          if (selectedExhibitor != null && !selectedExhibitors.contains(selectedExhibitor.id)) {
            ...
            print('✅ Exhibitor added: ${selectedExhibitor.name}');
          }
        },
      ),
    );
                ...
                children: [
                  ...
                  GestureDetector(
                    onTap: () {
                      ...
                      print('❌ Exhibitor removed: ${exhibitor.name}');
                    },
                    child: Icon(
                      Icons.close,
                      size: 14,
          ...
          children: [
            ...
            Switch(
              ...
              onChanged: (value) {
                ...
                print('🔄 Map visibility toggled: $value');
              },
          ...
          if (isLoading)
            Column(
              children: [
                ...
                SizedBox(height: 16),
                AppText(
                  text: isEditMode ? 'Editing...' : 'Adding...',
                  fontSize: 14,
                  color: AppColors.darkgrey,
                  fontWeight: FontWeight.w600,
                ),
  ...
  void _saveVenue() async {
    print('💾 Save venue button pressed');
    ...
    if (googleMapLinkController.text.isEmpty) {
      print('❌ Validation failed: Google Maps link is empty');
    ...
    print('📤 Event data prepared: ${eventData.toString()}');

    bool success = false;

    if (widget.eventId != null) {
      print('✏️ Updating event with ID: ${widget.eventId}');
      success = await _createViewModel.updateEvent(widget.eventId!, eventData);
    } else {
      print('➕ Creating new event');
      success = await _createViewModel.createEvent(eventData);
    }

    if (success) {
      final action = widget.eventId != null ? 'updated' : 'added';
      print('✅ Venue $action successfully');

      Get.snackbar(
        'Success',
        'Venue $action successfully!',
      ...
      // Go back to refresh the venue list
      print('🔙 Navigating back to venue list');
      Get.back(result: true); // Pass true to indicate success
    } else {
      print('❌ Failed to save venue');
      final action = widget.eventId != null ? 'update' : 'add';
      Get.snackbar(
        'Error',
        'Failed to $action venue. Please try again.',
        backgroundColor: Colors.red.withOpacity(0.1),
        colorText: Colors.red,
      );
    }
  ...
  @override
  void dispose() {
    print('🧹 Disposing AddNewVenueScreen');
