...
class _OrganizerVenueMapsScreenState extends State<OrganizerVenueMapsScreen> {
  ...
  void initState() {
    super.initState();
    print('📍 OrganizerVenueMapsScreen initialized');
    _loadVenueData();
  }
  ...
  // This will be called when returning from AddNewVenueScreen
  void _navigateToAddVenue({int? eventId}) async {
    print('➡️ Navigating to Add/Edit Venue Screen');
    final result = await Get.to(() => AddNewVenueScreen(eventId: eventId));

    // If result is true, it means venue was added/updated successfully
    if (result == true) {
      print('✅ Returned from Add/Edit screen with success, refreshing list...');
      _loadVenueData();
    } else {
      print('↩️ Returned from Add/Edit screen without changes');
    }
  }

  void _showDeleteDialog(OrganizerVenueShowEvent event) {
    print('🗑️ Delete dialog opened for: ${event.name}');
    showDialog(
      context: context,
        ...
        return AlertDialog(
          ...
          actions: [
            ...
            ElevatedButton(
              onPressed: () async {
                Navigator.of(context).pop();
                print('🗑️ Deleting event: ${event.name} (ID: ${event.id})');
                final success = await _deleteViewModel.deleteEvent(event.id);
                if (success) {
                  print('✅ Event deleted successfully, refreshing list...');
                  Get.snackbar(
                    'Success',
      ...
      body: RefreshIndicator(
        ...
        child: Obx(() {
          ...
          if (_venueViewModel.error.value.isNotEmpty) {
            print('❌ Error loading venues: ${_venueViewModel.error.value}');
            return Center(
              child: Column(
          ...
          print('✅ Loaded ${events.length} venues');
              ...
              children: [
                ...
                Padding(
                  ...
                  child: VenueMapComponent(
                    events: events,
                    onMarkerTap: (event) {
                      print('📍 Map marker tapped: ${event.name}');
                ...
                ListView.builder(
                  ...
                  itemBuilder: (context, index) {
                    return _buildLocationCard(events[index], index);
                  },
                ),
              ...
              children: [
                ...
                Expanded(
                  child: Column(
                    ...
                    children: [
                      ...
                      const SizedBox(height: 4),
                      AppText(
                        text: event.location ?? 'Latest Event is shown',
                        fontSize: 13,
                        color: AppColors.darkgrey,
                      ),
                    ],
                ...
                Column(
                  ...
                  children: [
                    InkWell(
                      onTap: () {
                        print('ℹ️ Details button pressed for: ${event.name}');
                        Get.to(() => OrganizerVenueDetails(event: event));
                      },
                      child: Container(
                    ...
                    InkWell(
                      onTap: () {
                        print('✏️ Edit button pressed for: ${event.name}');
                        _navigateToAddVenue(eventId: event.id);
                      },
                      child: Container(
                        padding: const EdgeInsets.symmetric(horizontal: 8, vertical: 4),
                        decoration: BoxDecoration(
  ...
  @override
  void dispose() {
    print('🧹 Disposing OrganizerVenueMapsScreen');
